...
const DOMAIN_DATA = [
  ...
  {
    id: "safehouse",
    title: "Safe House & Destination Plan",
    items: [
  ...
  {
    id: "family",
    title: "Family Coordination Plan",
    items: [
  ...
  {
    id: "health",
    title: "Health & Medical Plan (On Arrival)",
    items: [
  ...
  {
    id: "indicators",
    title: "Indications & Warning Signs",
    items: [
...
export default function ResilienceApp() {
  const [data, setData] = useLocalState(() => {
    ...
    DOMAIN_DATA.forEach((d) => {
      payload[d.id] = d.items.map(() => false);
    });
    return payload;
  });
  ...
  const current = useMemo(
    () => DOMAIN_DATA.find((d) => d.id === active),
    [active]
  );

  const progress = useMemo(() => {
    const totals = DOMAIN_DATA.map((d) => {
      const vals = data[d.id] || [];
      const done = vals.filter(Boolean).length;
      return { id: d.id, title: d.title, done, total: vals.length };
    });
  ...
  const filteredItems = useMemo(() => {
    ...
    return current.items
      .map((label, idx) => ({ label, idx, checked: vals[idx] }))
      .filter((item) =>
        filter === "all" ? true : filter === "open" ? !item.checked : item.checked
      );
  }, [current, data, filter]);
  ...
  const toggle = (idx) => {
    setData((prev) => ({
      ...prev,
      [current.id]: prev[current.id].map((v, i) => (i === idx ? !v : v)),
    }));
  };

  const resetDomain = () => {
    // eslint-disable-next-line no-restricted-globals
    if (!confirm(`Reset "${current.title}" checklist?`)) return;
    setData((prev) => ({
      ...prev,
      [current.id]: prev[current.id].map(() => false),
    }));
  };

  const resetAll = () => {
    // eslint-disable-next-line no-restricted-globals
    if (!confirm("Reset ALL domains?")) return;
    const fresh = {};
    DOMAIN_DATA.forEach((d) => {
      fresh[d.id] = d.items.map(() => false);
    });
    setData(fresh);
  };

  // ---------- Export / Import (CLEAN VERSION) ----------
  const exportJSON = () => {
    ...
    a.href = url;
    a.download = `resilience-checklists-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    ...
    <div className="min-h-screen bg-emerald-50 text-emerald-900 flex flex-col">
      ...
      <header className="sticky top-0 z-10 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white shadow">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-semibold tracking-wide">Resilience</h1>
          </div>
          <div className="flex items-center gap-2 text-emerald-50">
            <button
              title="Export"
              onClick={exportJSON}
              className="px-2 py-1 rounded hover:bg-emerald-600 flex items-center gap-1"
            >
            ...
            <button
              ...
              className="px-2 py-1 rounded hover:bg-emerald-600"
            >
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-4 grid md:grid-cols-[260px_1fr] gap-4">
        ...
        <aside className="bg-white rounded-2xl shadow p-3 border border-emerald-100">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-xl transition ${
                    sel ? "bg-emerald-600 text-white" : "hover:bg-emerald-50"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full ${
                        sel ? "bg-emerald-500/30" : "bg-emerald-100 text-emerald-700"
                      }`}
                    >
                      {pct}%
                    </span>
          ...
          <div className="mt-3 flex gap-2">
            <button
              onClick={resetDomain}
              className="flex-1 text-xs bg-white border border-emerald-200 text-emerald-700 rounded-lg py-1 hover:bg-emerald-50 flex items-center justify-center gap-1"
            >
            ...
            <button
              onClick={resetAll}
              className="flex-1 text-xs bg-white border border-emerald-200 text-emerald-700 rounded-lg py-1 hover:bg-emerald-50"
            >
              Reset all
            </button>
          </div>
        ...
        <section className="bg-white rounded-2xl shadow p-4 border border-emerald-100">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-emerald-800">{current.title}</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <ul className="space-y-2">
            {filteredItems.map((it) => (
              <li
                key={it.idx}
                className="flex items-center gap-3 p-3 rounded-xl border border-emerald-100 hover:bg-emerald-50"
              >
                <button
                  disabled={locked}
                ...
                <span
                  className={`text-sm ${
                    it.checked ? "line-through text-emerald-500" : "text-emerald-900"
                  }`}
                >
                  {it.label}
                </span>
          ...
          <div className="mt-4 text-right text-xs text-emerald-600 flex items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              {progress.totals.find((t) => t.id === current.id)?.done} /{" "}
              {progress.totals.find((t) => t.id === current.id)?.total} done
            </div>
      ...
      <footer className="bg-emerald-100 text-emerald-700">
        <div className="max-w-5xl mx-auto px-4 py-3 text-xs flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Leaf className="w-4 h-4" /> Resilience — peace & freedom
          </div>
      ...
      {/* Hidden anchors for simple pages */}
      <section id="about" className="hidden" />
      <section id="privacy" className="hidden" />
    </div>
  );
}
